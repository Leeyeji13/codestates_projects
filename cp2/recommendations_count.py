#상품에 대한 설명을 자연어 처리하여 이전에 구매한 아이템과 유사한 상품 추천하기(countvectorizer)
import pandas as pd
import numpy as np
...
count_artid = count_vectorizer.fit_transform((df['text']))
...
user_count = count_vectorizer.transform(q['text'])
cos_similarity_countv = map(lambda x: cosine_similarity(user_count, x), count_artid)

cos_sim_list = list(cos_similarity_countv)

top_10 = sorted(range(len(cos_sim_list)), key=lambda i: cos_sim_list[i], reverse=True)[:10]
list_scores_cv = [cos_sim_list[i][0][0] for i in top_10]
pd.set_option('display.max_columns', None)
print(recommendation_product(top_10, df, list_scores_cv))